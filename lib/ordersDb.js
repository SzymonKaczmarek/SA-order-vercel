...
function extractStatusLabel(order) {
  if (order?.status && typeof order.status === 'object') {
    return String(order.status.name || order.status.id || '');
  }
  return String(order?.status_name || order?.status_id || order?.status || '');
}
...
async function insertOrdersBatch(scopeKey, orders) {
  if (!orders.length) {
    return 0;
  }
  ...
  for (let i = 0; i < orders.length; i += BATCH_SIZE) {
    ...
    await store.sql`
      ...
      FROM unnest(
        ...
        ${payloads}::jsonb[],
        ${statuses}::text[]
      ) AS batch(sellasist_id, payload, status_label)
...
async function getOrders(scopeKey, { offset, limit } = {}) {
  ...
  if (store.useFallback) {
    ...
    const allOrders = Array.from(bucket.values());
    if (!hasPaging) {
      return { ...base, orders: allOrders };
    }
