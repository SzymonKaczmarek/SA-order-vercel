        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 space-y-1">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              {isConfirm
                ? `W bazie znajdują się już pobrane produkty${totalItems > 0 ? ` (${totalItems})` : ''}. Czy je załadować?`
                : label}
            </p>
          </div>
        </div>
        ...
        {isConfirm ? (
          <div className="flex flex-wrap items-center justify-end gap-2 pt-1">
            <button
              ...
              onClick={onDecline}
              disabled={loading}
              className="rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50"
            >
              Nie
            </button>
            <button
              ...
              onClick={onConfirm}
              disabled={loading}
              className="rounded-xl bg-brand-primary px-4 py-2.5 text-xs font-semibold text-white hover:opacity-95 disabled:opacity-50"
            >
              Tak, załaduj
            </button>
          </div>
